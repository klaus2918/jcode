...
> **注（2026-08-16）**：文中引用的测量/分析工具脚本（`compile_time_probe.sh` / `compile_isolation_report.py` / `bench_compile.sh` / `bench_selfdev_checkpoints.sh` / `bench_startup.py` / `check_startup_budget.sh`）已随 feature-simplification 精简移除。本文件的迁移方法论与目标保持有效；如需重新测量编译时间，请按本文思路重建探针。
...
对于移动代码的阶段，还运行被移动领域的相关定向测试，并在可行时通过协调的 selfdev 路径做一次完整 selfdev 构建：
