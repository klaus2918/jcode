...
`MemoryEntry`（`crates/jcode-memory-types/src/lib.rs`）的字段分四组：

- **身份与分类**：`id`、`content`、`category`（`Fact` / `Preference` / `Entity` /
  `Correction` / 自定义字符串）、`tags`、`search_text`（预归一化的检索文本）；
- **来源**：`source`（写入时记录的来源，memory 工具写入的是会话 id）、
  `reinforcements`（`Vec<Reinforcement>`：`session_id` / `message_index` / `timestamp`）；
- **生命周期**：`created_at`、`updated_at`、`access_count`、`strength`（被强化次数）；
- **信任与状态**：`trust`（`high` / `medium` / `low`）、`confidence`、`active`、
  `superseded_by`、`embedding`、`embedding_model`（嵌入模型标识）。

作用域（`project` / `global` / `all`）不是条目字段，而是存储层与工具参数的维度：
条目本身落在项目图或全局图里。

（历史字段 `memory_type`、`provenance`、`last_accessed`、`trust_score`、`message_range`、
`file_paths` 在当前实现中不存在。）
...
```
effective_confidence = min(1.0, confidence * e^(-age_days/half_life * ln2)
                                 * (1 + 0.1 * ln(access_count + 1)))
```
...
| Entity | 60 天 | 实体变化适中 |
| Custom | 45 天 | 自定义分类的兜底值 |
| Fact | 30 天 | 代码库事实会过期 |

信任权重不在置信度里，而在召回排序 `memory_score()`（同一 crate）中相乘：

```
score = (100 / (1 + age_hours/24)          # 近期性（按 updated_at）
         + sqrt(access_count) * 10          # 使用频率
         + 分类加成)                        # Correction 50 / Preference 30 / Fact 20 / Entity 10 / Custom 5
        * trust_weight                      # high 1.5 / medium 1.0 / low 0.7
        + ln(strength) * 5                  # 强化次数
```

非活跃条目（`active == false`）得 0 分。
...
| 参数 | 实际值 | 含义 |
|---|---:|---|
| 相似度阈值 | 0.5 | 初始嵌入命中的最小相似度（工具调用时传入） |
| `limit` | 10（工具参数默认） | 初始命中条数，也是最终返回条数 |
| BFS 深度 | 2（硬编码） | `cascade_retrieve(seed, score, 2, limit * 2)` |
| 每图候选上限 | `limit * 2`（默认 20） | 两张图各自保留的候选数，合并后按分数取前 `limit` 条 |
| 每步衰减 | `0.7^(depth+1)`（硬编码） | 边权之外的距离衰减 |

边权（`EdgeKind::traversal_weight`）：`HasTag` 0.8、`InCluster` 0.6、`RelatesTo` 取边上的权重、
`Supersedes` 0.9、`DerivedFrom` 0.7、`Contradicts` 0.3。
...
```
~/.jcode/memory/
├── global.json                  # 用户级记忆图（MemoryGraph）
├── projects/<project_hash>.json # 按工作目录划分的记忆图
└── test/                        # 测试模式的隔离存储（clear-test 清这里）
```

标签、集群、语义边与嵌入都写在**同一个**图文件里（`MemoryGraph` 的 `memories` / `tags` /
`clusters` / `edges` 字段，嵌入是条目上的 `embedding` 内联数组），没有单独的
`embeddings/`、`clusters/`、`tags/` 目录。旧格式（`MemoryStore`）首次读取时自动迁移，
并留下 `<hash>.json.bak` 备份；更早的 `~/.jcode/notes/<hash>.json` 也会被导入图。

智能体工具：

```
memory { action: "remember", content, category, scope, tags }
memory { action: "recall" }                       # 取相关记忆；mode 默认 recent，有 query 时默认 cascade
memory { action: "search", query }                # 语义搜索
memory { action: "list", tag? }
memory { action: "forget", id }
memory { action: "tag", id, tags }
memory { action: "link", from_id, to_id, weight }
memory { action: "related", id, depth }           # 沿图取相关记忆（depth 默认 2）
```

`recall` 的 `mode` 取值为 `recent` / `semantic` / `cascade`。
...
| 指标 | 上限 | 来源 |
|---|---:|---|
| Markdown 高亮缓存条目 | 256（`HIGHLIGHT_CACHE_LIMIT`） | `crates/jcode-tui-markdown/src/lib.rs` |
| Mermaid 渲染缓存条目 | 512（`RENDER_CACHE_MAX`） | `crates/jcode-tui-mermaid/src/mermaid_cache_render.rs` |
| Mermaid 协议图像状态条目 | 24（`IMAGE_STATE_MAX`） | `crates/jcode-tui-mermaid/src/lib.rs` |
| Mermaid 解码源缓存条目 | 16（`SOURCE_CACHE_MAX`） | 同上 |
| Mermaid 活动图 | 128（`ACTIVE_DIAGRAMS_MAX`） | 同上 |
| Mermaid 磁盘缓存 | 50 MiB（`CACHE_MAX_SIZE_BYTES`）/ 最长 3 天（`CACHE_MAX_AGE_SECS`） | 同上 |
