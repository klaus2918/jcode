...
Today, the server-side "claimed"/"Delivered" state can happen while building a
History payload, before the TUI has actually processed that payload. This script
flags sessions that look claimed/delivered but have no matching TUI-side queue
log.
...
HISTORY_SNAPSHOT_RE = re.compile(
    r"history_reload_recovery_snapshot: using server-owned recovery intent for session=([^ ]+)"
)
...
class IntentRecord:
    ...
    def verdict(self) -> str:
        ...
        if self.claimed or (self.durable_status or "").lower() == "delivered":
            return "suspect-claimed-without-client-queue-log"
        if self.persisted:
            return "persisted-not-claimed"
...
def render_table(records: list[IntentRecord], hidden_sends: list[Event], show_lines: bool) -> str:
    ...
    out.append(f"Suspect claimed-without-client-queue records: {len(suspects)}")
    ...
    if suspects:
        ...
        for record in suspects:
            ...
            out.append(f"  - {record.session_id} ({record.role}) {record.reload_id} claim={claim}")
