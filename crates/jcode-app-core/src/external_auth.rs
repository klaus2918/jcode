...
pub struct ExternalAuthAutoImportOutcome {
    ...
    /// Providers that were successfully imported, in the order they were
    /// applied. Used by `preferred_activation_provider` to pick the default
    /// provider to activate after a multi-source import.
...
/// became available.
pub fn import_provider_labels(candidate: &ExternalAuthReviewCandidate) -> Vec<&'static str> {
    ...
    for &index in selected {
        ...
        match validate_external_auth_review_candidate(candidate).await {
            Ok(detail) => {
                ...
                outcome
                    .imported_providers
                    .extend(import_provider_labels(candidate));
...
mod render_markdown_tests {
    ...
    fn groups_imported_and_skipped_with_counts() {
        let outcome = ExternalAuthAutoImportOutcome {
            ...
            imported_providers: vec!["openai", "claude"],
        };
    ...
    fn preferred_activation_provider_is_deterministic_for_multi_import() {
        let outcome = ExternalAuthAutoImportOutcome {
            imported: 3,
            messages: Vec::new(),
            imported_providers: vec!["gemini", "claude"],
        };
