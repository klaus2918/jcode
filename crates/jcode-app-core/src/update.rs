...
///
/// 与从官网下载的路径共用同一套安装管线（versions 归档 + stable/current 通道 +
/// launcher 切换），只是安装包来源是本机文件而非 GitHub release。
pub fn install_local_artifact_blocking(
        ...
        .is_some_and(|name| name.to_string_lossy().ends_with(".tar.gz"))
    {
        ...
        // 文件，源文件与已安装版本彻底解耦（与官网下载路径的临时文件做法一致）。
    ...
    let mut metadata = UpdateMetadata::load().unwrap_or_default();
    // 与 download_and_install_blocking_with_progress 一致：installed_version
    // 记版本号本身（不带 v 前缀），installed_from 记本地安装包路径便于审计。
...
///
/// 供官网下载与本地安装包两条路径共用：输入都是发布包字节，输出都是已安装的
/// 版本化二进制（launcher/通道切换由调用方负责）。
fn install_tar_gz_archive_blocking(archive_bytes: &[u8], version: &str) -> Result<PathBuf> {
    ...
    for entry in fs::read_dir(&extract_dir).context("Failed to read extracted archive")? {
        ...
        // 官网资产名与 get_asset_name() 完全一致；本地安装包常带 git hash
        // 后缀（如 jcode-windows-x86_64-2dc3213a6.exe）。凡是以 jcode 开头、
        // 不是 .bin payload 的顶层文件都视为主二进制，统一重命名为标准名。
...
pub fn download_and_install_blocking_with_progress(
    ...
    if asset.name.ends_with(".tar.gz") {
        ...
        installed_version_dir = Some(install_tar_gz_archive_blocking(&bytes, version)?);
    } else {
