...
fn test_login_picker_preview_stays_open_and_updates_filter() {
    ...
    assert_eq!(picker.filter, "op");
    assert!(
        picker
            .filtered
            .iter()
            .any(|&i| picker.entries[i].name == "OpenAI-compatible")
    );
    assert_eq!(app.input(), "/login op");
}
...
fn test_login_picker_preview_enter_starts_login_flow() {
    ...
    for c in "/login openai-compatible".chars() {
        app.handle_key(KeyCode::Char(c), KeyModifiers::empty())
            .unwrap();
    }
    app.handle_key(KeyCode::Enter, KeyModifiers::empty())
