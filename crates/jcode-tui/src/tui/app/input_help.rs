    ...
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            "fast-release" | "cut-release" | "commit-push-release" => {
                "/fast-release\nSame as /commit-push, then publish the release.\n\nThe agent picks the semver bump, updates Cargo.toml/Cargo.lock and the changelog, commits and pushes, then runs scripts/quick-release.sh to tag and push the tag. This fork ships Windows x86_64 only and builds it in GitHub Actions, so there is no local build step: the pushed tag starts the build, signing, checksums, and publication. /cut-release and /commit-push-release are compatibility aliases."
            }
            "remote-release" => {
                "/remote-release\nSame as /commit-push, then push the release tag without running any local build.\n\nThe agent picks the semver bump, updates Cargo.toml/Cargo.lock and the changelog, commits and pushes, then runs scripts/quick-release.sh (its former --remote flag was removed on 2026-09-13; the script now has a single mode). GitHub Actions builds, signs, checksums, and publishes the Windows x86_64 assets; the release remains a draft until the remote gates pass."
            }
            "triage" => {
