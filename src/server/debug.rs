...
async fn spawn_tester(opts: serde_json::Value) -> Result<String> {
    ...
    // Restrict tester temp files to owner-only to prevent other local users
    // from reading tool output (which may contain API responses) or injecting
    // commands via the debug IPC files.
    let _ = crate::platform::set_permissions_owner_only(&stdout_path);
    let _ = crate::platform::set_permissions_owner_only(&stderr_path);
    // Pre-create the IPC files with restricted permissions before passing
    // paths to the child process, so the child can't race to create them.
