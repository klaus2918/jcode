...
pub async fn run_self_dev(should_build: bool, resume_session: Option<String>) -> Result<()> {
    ...
    if !server_running {
        super::dispatch::maybe_prompt_server_bootstrap_login("auto").await?;
        super::dispatch::spawn_server("auto", None, None).await?;
    }
    ...
    super::tui_launch::run_tui_client(Some(session_id), !server_running, false, None, false).await
}
#[cfg(test)]
